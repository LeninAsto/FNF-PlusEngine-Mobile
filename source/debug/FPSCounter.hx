...
class FPSCounter extends TextField
{
    ...
    public function new(x:Float = 10, y:Float = 10, color:Int = 0x000000)
    {
        ...
        var vcrFont = null;
        #if (backend_Paths || Paths)
        try vcrFont = Paths.defaultFont() catch(e:Dynamic) vcrFont = null;
        #end
        if (vcrFont == null) vcrFont = "_sans";
    ...
    public dynamic function updateText():Void // so people can override it in hscript
    {
        ...
        if (Reflect.hasField(ClientPrefs.data, "showStateInFPS") && ClientPrefs.data.showStateInFPS) {
            var stateName = "";
            if (FlxG.state != null)
